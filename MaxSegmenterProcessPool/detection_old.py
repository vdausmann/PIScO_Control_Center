...
def detect_on_img(input, settings: DetectionSettings, mask: np.ndarray, index=0):
    ...
    if mean_raw[1]>2:
        ...
        for i, cnt in enumerate(cnts):
            ...
            mask_fn = os.path.join(settings.crop_path, f"{fn[:-4]}_{c}_mask.jpg")
            ...
            if areas[i] > settings.min_area_to_save:
                ...
                if settings.save_crops:
                    ...
                    cv.imwrite(mask_fn,crop_mask)
            else:
                crop_data.append([c, os.path.basename(crop_fn), mean_raw[0],mean_raw[1],mean,std, areas[i], x, y, w, h, 0])
