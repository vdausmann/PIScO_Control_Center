...
### old forward function
    ...
    def forward(self, x):
        correction_1 = self.correction_branch[0](x)
        correction_1 = self.Encoder[0](correction_1)

        correction_2 = self.correction_branch[1](correction_1)

        update_1 = self.update_branch[0](x)
        update_1 = self.Encoder[1](update_1)

        update_2 = self.update_branch[1](update_1)

        bottleneck_1 = torch.cat([correction_2, update_2], dim=1)
        bottleneck_1 = self.bottleneck[0](bottleneck_1)
        bottleneck_1 = self.Encoder[2](bottleneck_1)

        # ----
        res0 = F.interpolate(correction_1, scale_factor=0.5)
        res1 = F.interpolate(bottleneck_1, scale_factor=2)

        res1 = self.AFFs[0](correction_1, res1)
        res2 = self.AFFs[1](bottleneck_1, res0)
        # ----

        bottleneck_1 = self.bottleneck[1](res2)
        bottleneck_1 = self.Decoder[0](bottleneck_1)
        bottleneck_1 = self.bottleneck[2](bottleneck_1)


        correction_2 = torch.cat([res1, bottleneck_1], dim=1)
        correction_2 = self.correction_branch[2](correction_2)
        correction_2 = self.Decoder[1](correction_2)
        correction_2 = self.correction_branch[3](correction_2)

        y_k = x + correction_2

        update_2 = self.update_branch[2](x, update_1) # RL division

        update_3 = torch.cat([update_2, bottleneck_1], dim=1)

        update_3 = self.update_branch[3](update_3)
        update_3 = self.Decoder[2](update_3 + update_2)
        update_3 = self.update_branch[4](update_3)

        update_3 = torch.mean(update_3, dim=1, keepdim=True)

        y = self.up(y_k * update_3)

        return y, y_k, update_3