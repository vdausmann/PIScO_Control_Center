		...
		std::unordered_map<size_t, std::vector<Crop>>& tiles) 
{
	...
	size_t rowSize = 0;
	size_t imageIdx = 0;
	for (auto& [key, crops]: tiles) {
		for (Crop& crop: crops) {

			// wrap column
			if (currentCol + crop.image.cols + padding >= w) {
				currentCol = padding;
			...
			// new image
			if (currentRow + crop.image.rows + padding >= h) {
			...
			cv::Rect region(currentCol, currentRow, crop.image.cols, crop.image.rows);
			crop.image.copyTo(cropGroup(region));
			crop.tile = region;

			currentCol += crop.image.cols + padding;
			if (crop.image.rows > rowSize) rowSize = crop.image.rows;
		}
	}
	...
	std::cout << "Compressed " << tiles.size() << " full images to " << 
