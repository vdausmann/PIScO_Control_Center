...
std::string e_dateTime;
std::string e_profileName;
std::string e_imageSourcePath;
...
void readParameters(char* filename)
{
	...
	readParameter(fileConfig, e_sourceFile, VAR_NAME(e_sourceFile));
	...
	size_t idx = e_sourceFile.find_last_of('/');
	e_profileName = e_sourceFile.substr(idx + 1, e_sourceFile.size());
